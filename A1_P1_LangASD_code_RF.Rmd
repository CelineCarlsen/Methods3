...
Set the working directory (the directory with your data and code for these assignments): bla bla

```{r}
setwd('~/Dropbox (Personal)/My courses/2017 - Methods 3/Assignments/')
library(pacman)
p_load(readr,dplyr,stringr)
```
...
```{r}
Speech$Child.ID = str_replace_all(Speech$Child.ID,c(
  "Adam." = "Adam", 
  "Albert." = "Albert",
  "Alfie."  = "Alfie",
  "Allison." = "Allison",
...
It would be smart to rename the Mullen and ExpressiveLanguage into something you can remember (i.e. nonVerbalIQ, verbalIQ)

```{r}

Demo=select(Demo,c("Child.ID", "Visit", "Ethnicity", "Diagnosis", "Gender", "Age", "ADOS", "MullenRaw", "ExpressiveLangRaw"))
Speech=select(Speech,c("Child.ID", "Visit", "MOT_MLU", "MOT_LUstd", "CHI_MLU", "CHI_LUstd", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI"))
...
Data1= Data %>% subset(Visit=="1") %>% rename(Ados1 = ADOS, verbalIQ1=ExpressiveLangRaw, nonVerbalIQ1=MullenRaw) %>% select(-Visit)
